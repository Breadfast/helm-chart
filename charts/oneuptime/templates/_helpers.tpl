...
{{/*
Name of the ServiceAccount to attach to the app and worker pods.
Returns .Values.serviceAccount.name when set; otherwise falls back to the
release name. This is referenced both by the ServiceAccount template and by the
app/worker pod specs so a created SA and its consumers always agree on the name.
*/}}
...
# Google Chat app. PROJECT_NUMBER is the `aud` the inbound-request verifier
# (GoogleChatAuthorization) enforces; it MUST equal the Chat app's Cloud project
# number AND the Console "Authentication Audience". CUSTOMER_ID is only used by
# the (dormant) Tier-B space-create path.
- name: GOOGLE_CHAT_PROJECT_NUMBER
  value: {{ $.Values.googleChat.projectNumber | quote }}
- name: GOOGLE_CHAT_CUSTOMER_ID
